...
void btSoftBody::PSolve_RContacts(btSoftBody* psb, btScalar kst, btScalar ti)
{
	...
	for (int i = 0, ni = psb->m_rcontacts.size(); i < ni; ++i)
	{
		...
		if (cti.m_colObj->hasContactResponse())
		{
			...
			btScalar* deltaV = nullptr;//EDIT BY ALİ OSMAN ŞAHİN TO AVOID UNINITIALIZED WARNING
			...
			if (dn <= SIMD_EPSILON)
			{
				...
				{
					if (multibodyLinkCol && deltaV) //EDIT BY ALİ OSMAN ŞAHİN TO AVOID UNINITIALIZED WARNING
					{
