...
unsigned int Graphics::LoadTexture(const char* id, const char* path, int& width, int& height)
{
    ...
    adr::adr_glGenTextures(1, &texture);
    adr::adr_glBindTexture(GL_TEXTURE_2D, texture);

    //texture parameters
    adr::adr_glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    adr::adr_glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
    ...
    if (data) {
        ...
        adr::adr_glTexImage2D(GL_TEXTURE_2D, 0, format, width, height, 0, format, GL_UNSIGNED_BYTE, data);

        //generates mipmaps
        adr::adr_glGenerateMipmap(GL_TEXTURE_2D);
    }
    else {
...
{
    adr::adr_glDeleteTextures(1, &texture);
}
...
GRAPHICS_API void Graphics::UnloadMesh(std::vector<std::shared_ptr<ObjectMtl>>& objects)
{
    for (auto& object : objects) {
        adr::adr_glDeleteTextures(1, &object->material.diffuseTexture);
        adr::adr_glDeleteBuffers(1, &object->VBO_positions);
        adr::adr_glDeleteBuffers(1, &object->VBO_normals);
        adr::adr_glDeleteBuffers(1, &object->VBO_uvs);
        adr::adr_glDeleteVertexArrays(1, &object->VAO);
    }
}
...
GRAPHICS_API bool Graphics::LoadMesh(const char* id, const char* path, std::vector<std::shared_ptr<ObjectMtl>>& out_objectMtls)
{
    ...
    for (const auto& shape : shapes) {
        ...
        for (const auto& pair : materialObjects) {
            ...
            // VAO / VBO setup
            adr::adr_glGenBuffers(1, &object->VBO_positions);
            adr::adr_glGenBuffers(1, &object->VBO_normals);
            adr::adr_glGenBuffers(1, &object->VBO_uvs);
            adr::adr_glGenVertexArrays(1, &object->VAO);
            adr::adr_glBindVertexArray(object->VAO);

            if (!object->vertexIndices.empty()) {
                adr::adr_glBindBuffer(GL_ARRAY_BUFFER, object->VBO_positions);
                adr::adr_glBufferData(GL_ARRAY_BUFFER, object->vertexIndices.size() * sizeof(glm::vec3), &object->vertexIndices[0], GL_STATIC_DRAW);
                adr::adr_glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(glm::vec3), (void*)0);
                adr::adr_glEnableVertexAttribArray(0);
            }

            if (!object->uvIndices.empty()) {
                adr::adr_glBindBuffer(GL_ARRAY_BUFFER, object->VBO_uvs);
                adr::adr_glBufferData(GL_ARRAY_BUFFER, object->uvIndices.size() * sizeof(glm::vec2), &object->uvIndices[0], GL_STATIC_DRAW);
                adr::adr_glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, sizeof(glm::vec2), (void*)0);
                adr::adr_glEnableVertexAttribArray(1);
            }

            if (!object->normalIndices.empty()) {
                adr::adr_glBindBuffer(GL_ARRAY_BUFFER, object->VBO_normals);
                adr::adr_glBufferData(GL_ARRAY_BUFFER, object->normalIndices.size() * sizeof(glm::vec3), &object->normalIndices[0], GL_STATIC_DRAW);
                adr::adr_glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, sizeof(glm::vec3), (void*)0);
                adr::adr_glEnableVertexAttribArray(2);
            }

            adr::adr_glBindBuffer(GL_ARRAY_BUFFER, 0);
            adr::adr_glBindVertexArray(0);
...
{
    adr::adr_glViewport(0, 0, width, height);
}
