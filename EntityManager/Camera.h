...
public:
	...
	*/
	...
	//Camera specs
	ENTITYMANAGER_API void AddPosition(glm::vec3 posAdd, bool isPlaying) override;
	ENTITYMANAGER_API void AddRotation(float _yaw, float _pitch, bool isPlaying) override;

	ENTITYMANAGER_API void SetPosition(glm::vec3 newPos, bool isPlaying) override;
	ENTITYMANAGER_API void SetRotation(float _yaw, float _pitch, bool isPlaying) override;

	//first -> yaw, second -> pitch
	ENTITYMANAGER_API std::pair<float, float> GetYawPitch() override;

	//Getters for parameters
	ENTITYMANAGER_API CameraProjection GetProjectionType() override { return params->GetProjectionType(); };
	ENTITYMANAGER_API float            GetFOV() override { return params->GetFOV(); };
private:
	//properties
	std::shared_ptr<ICameraParams> params;
